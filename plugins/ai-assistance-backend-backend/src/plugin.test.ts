...
import { aiAssistanceBackendPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [aiAssistanceBackendPlugin],
    });

    await request(server).get('/api/ai-assistance-backend/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/ai-assistance-backend/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/ai-assistance-backend/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/ai-assistance-backend/todos/${createdTodoItem.id}`)
